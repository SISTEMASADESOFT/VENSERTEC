var AppSession = "../Ventas/RegistroCotizacionLUBRISERVICIOS.aspx";
...
//            F_InicializarCorreos();
    ...
    if (!F_SesionRedireccionar(AppSession)) return false;
    if (F_PermisoOpcion(CodigoMenu, '4000101', '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try 
        {
...
//                MostrarEspera(false);
    ...
    if (!F_SesionRedireccionar(AppSession)) return false;
    if (F_PermisoOpcion(CodigoMenu, '4000102', '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try 
        {
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        $('#MainContent_ddlTipoDoc').val(15);
                        $('#MainContent_ddlTipoDoc2').val(15);
                        $('#MainContent_ddlTipoDoc').css('background', '#FFFFE0');
                        $('#MainContent_ddlTipoDoc2').css('background', '#FFFFE0');
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        $('.ccsestilo').css('background', '#FFFFE0');
                        $('#MainContent_grvConsultaArticulo_imgAgregar_0').focus();
...
//                                  if (parseFloat(txtcantidad_grilla) > parseFloat(lblStock_grilla) && parseFloat(lblStock_grilla)===0.00 )
...
//                    Cadena=Cadena + '<p></p>' + 'Numero de Factura';
             ...
             if ($(Cuerpo + 'txtTotal').val()=='0.00')
                 ...
                 if (( $(Cuerpo + 'txtPlaca').val()+ $(Cuerpo + 'txtPlaca2').val() +$(Cuerpo + 'txtPlaca3').val()+ $(Cuerpo + 'txtPlaca4').val())!='')
                                {
                         if((($(Cuerpo + 'txtPlaca').val()==$(Cuerpo + 'txtPlaca2').val()) || ($(Cuerpo + 'txtPlaca').val()==$(Cuerpo + 'txtPlaca3').val())
                         || ($(Cuerpo + 'txtPlaca').val()==$(Cuerpo + 'txtPlaca4').val())  || ($(Cuerpo + 'txtPlaca2').val()==$(Cuerpo + 'txtPlaca3').val())
                         || ($(Cuerpo + 'txtPlaca2').val()==$(Cuerpo + 'txtPlaca4').val()) || ($(Cuerpo + 'txtPlaca3').val()==$(Cuerpo + 'txtPlaca4').val())
                         ) && ($(Cuerpo + 'txtPlaca').val()!=''&& $(Cuerpo + 'txtPlaca2').val()!='' && $(Cuerpo + 'txtPlaca3').val()!=''&& $(Cuerpo + 'txtPlaca4').val()!=''))
                        {
       ...
       try 
        {
              ...
              var objParams = {
                ...
                if (str_resultado_operacion == "1") 
                {
                    ...
                    $('#lblGrillaEspera').text(F_Numerar_Grilla("grvConsulta",'lblnumero'));
                    if (str_mensaje_operacion!='')                           
                    alertify.log(str_mensaje_operacion);
...
function F_AnularPopUP(Fila) {
        ...
        if ($(lblEstado).text() != "PENDIENTE") {
            alertify.log("SOLO LA COTIZACIONES PENDIENTES SE PUEDEN ANULAR");
            return false;
        }
 ...
 try 
        {
         ...
         if(!confirm("ESTA SEGURO DE ANULAR LA COTIZACION : " + $('#hfNumeroAnulacion').val() + "\n" + "DEL CLIENTE : " +  $('#hfClienteAnulacion').val()))
    return false;
...
function F_EditarRegistro(Fila) {
 ...
 try 
    ...
    F_DatosFactura_NET(arg, function (result) {
                    ...
                    $('#MainContent_txtObservacion').val(result.split('~')[32]);
...
function F_ImpresionGrilla(Fila,rplc,TipoImp,NroCopias) {
    ...
    if (F_PermisoOpcion(CodigoMenu, '4000105', '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
...
function F_ImpresionDefinitiva(CodMenu, Codigo, SerieDoc, CodTipoDoc, TipoImp, NroCopias) {
    ...
    var NombreArchivo = 'rptCotizacion.rpt';
    rptURL = '../Reportes/Crystal.aspx';
    rptURL = rptURL + '?';
    rptURL = rptURL + 'CodMenu=' + CodMenu + '&';
 ...
 try 
        {
            ...
            {
            alertify.log("El precio no puede menor o igual a 0");
            ...
            return false;
               ...
               if($(txtPrecio).val()=='')
            {
            alertify.log("El precio no puede ser vacio");
            $(txtPrecio).val(parseFloat($(hfPrecio).val()).toFixed(2));
            return false;
            }
 ...
 try 
        {
            ...
            {
            alertify.log("La cantidad no puede menor o igual a 0");
            ...
            return false;
                ...
                if($(txtCantidad).val()=='')
            {
            alertify.log("La cantidad no puede ser vacio");
            $(txtCantidad).val(parseFloat($(hfCantidad).val()).toFixed(2));
            return false;
            }
...
function F_LlenarGridDetalle_Observacion(Fila){
  try 
        {             
                ...
                HfgvAuditoria = '#' + Fila.replace('pnlOrdersObservacion', 'hfDetalleCargado');
...
function F_ACTUALIZAR_MONTO_MONEDA(){
  try 
        ...
        if($(Contenedor + 'ddlMoneda').val()==1 && TC==0) {
                    TC = $('#MainContent_lblTCParalelo').text(); 
                    }
                    else if ($(Contenedor + 'ddlMoneda').val()==2 && TC==0){
                     TC = $('#MainContent_lblTCParaleloCompra').text();
                    }
...
function F_ReemplazarDocumento(Fila) {
        ...
        try {
            ...
            if ($(lblEstado).text() != 'PENDIENTE') {
                alertify.log('SOLO SE PUEDEN MODIFICAR COTIZACIONES PENDIENTES');
                return true;
            }

            var objParams = {
                Filtro_CodDoc: 15,
                Filtro_CodProforma: $(lblcodigo).val(),
                Filtro_TasaIgv: parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1),
                Filtro_NotaPedido: 0,
...
function F_CamposAlternativas(Campos) { 
                switch (Campos)
                {
                    case "MainContent_txtArticulo":
                        $('#MainContent_btnBuscarArticulo').select();
                    break;
