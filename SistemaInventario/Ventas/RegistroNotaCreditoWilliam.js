var AppSession = "../Ventas/RegistroNotaCreditoWilliam.aspx";
var CodigoMenu = 4000; var CodigoInterno = 6;
...
//Bond
var ImpresoraPDF = 'EPSON L395 Series';
var NroCopiasPDF = '2';
//Matricial
...
//---------------------------- 
var CodigoEmpresa = 4; //1 Vensertec VR //2 Lubricentro //3 Vensertec SP //4 Lubriservicio
...
if (!F_SesionRedireccionar(AppSession)) return false;
    ...
    $('#MainContent_btnFactura').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
    if (F_PermisoOpcion(CodigoMenu, '777009', '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
...
function F_ValidarGrabarDocumento() {
    try {
        ...
        if ($('#MainContent_ddlTipoDocumento').val() == "1" & $("#MainContent_ddlSerie option:selected").text().substring(0, 1) == 'B')
            Cadena = Cadena + '<p></p>' + 'LA SERIE DE LA NOTA DE CREDITO PARA LAS FACTURAS EMPIENZAN CON F Y DE LAS BOLETAS CON B';
...
function F_FacturaNotaCredito() {
    ...
    try {
        var objParams = {
            ...
            Filtro_CodTipoOperacionNC: $('#MainContent_ddlTipoOperaciones').val(),
        ...
        F_FacturacionNotaCredito_NET(arg, function (result) {
            ...
            if (str_mensaje_operacion == "") {
                $('#hfCodigoTemporal').val(result.split('~')[2]);
                 $('#hfCodDistrito').val(result.split('~')[13]);
                  $('#hfCodProvincia').val(result.split('~')[14]);
                  $('#hfCodDepartamento').val(result.split('~')[12]);
                ...
                $('#MainContent_txtNumeroRelacionado').val($('#MainContent_txtCodFactura').val());
                ...
                $('#MainContent_ddlSerie').val($('#MainContent_ddlSerie option:first').val());

                if ($('#MainContent_txtCodFactura').val().split('-')[0] != $('#MainContent_ddlSerie option:selected').text())
                    $('#MainContent_ddlSerie').val($('#MainContent_ddlSerie option:selected').next().val()); 
                
                if ($('#MainContent_txtCodFactura').val().split('-')[0] != $('#MainContent_ddlSerie option:selected').text())
                    $('#MainContent_ddlSerie').val(''); 
                ...
                if ($('#MainContent_ddlFormaPago').val() == 1)
                    $('#MainContent_txtAcuenta').val(parseFloat(parseFloat($('#MainContent_txtTotal').val()) - parseFloat($('#MainContent_txtAcuentaNV').val())).toFixed(2));
                        ...
                        $('#lblNroItems').text(F_Numerar_Grilla("grvDetalleArticulo", 'lblCodigo'));
...
//        if (parseFloat($(Contenedor + 'txtAcuentaNV').val()) == parseFloat($(Contenedor + 'txtTotal').val()))
//            return false;
...
function F_EliminarRegistro(Fila) {
    try {
        ...
        if (!confirm("ESTA SEGURO DE ELIMINAR LA NOTA DE CREDITO : " + $(lblNumero).text() + "<p></p>" + "DEL CLIENTE : " + $(lblCliente).text()))
            return false;
...
//                        NroCopiasMTX = '1';
//                        MargenInferior = 0;
//                        ArchivoRPT = 'FacturaLubricentro.rpt';
//                        NombreTabla='Electronica';
//                        NombreArchivo= 'FacturaLubricentro.rpt';
//                        if (nrodoc.substr(0, 3) != '001')
//                            return false;
//                        break;
//                }
//                break;
...
function F_ContarItems(){
 ...
 try 
        ...
        var x=0;
                ...
                $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
                    chkSi = '#' + this.id;
                    var nro = chkSi.replace('chkEliminar', 'lblCodDetalle');          
...
function F_ImprimirFacturaHTML(Fila) {
    ...
    F_ImprimirFactura($(lblCodigo).val(),CodMenu,TipoImp);
    return false;
}
