var AppSession = "../Ventas/OrdenVenta.aspx";
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        $('#hfCodSede').val(result.split('~')[11]);
                        $('#MainContent_ddlMoneda').val(1);
                        $('#MainContent_ddlFormaPago').val(11);
                        $('#MainContent_txtVencimiento').val($('#MainContent_txtEmision').val());
                        F_Update_Division_HTML('div_MonedaComision', result.split('~')[10]);
                        $('#MainContent_ddlMonedaComision').val(1);
  ...
  try 
        ...
        var RazonSocial = $('#MainContent_txtCliente').val();
                ...
                var objParams = {
                                        ...
                                        Filtro_SerieOC: $(Contenedor + 'txtSerieOC').val(),
                                        Filtro_NumeroOC: $(Contenedor + 'txtNumeroOC').val(),
...
function F_AnularRegistro(Fila) {
 try 
    ...
    if ($(lblestado_grilla).text()=="Anulado(a)") 
...
//    $('#MainContent_ddlSerieConsulta').val(62);
...
function F_GrabarComision(){
  ...
  try 
        ...
        {alert ("La factura tiene una comision registrada"); return false;}

        if ($(Contenedor + 'txtComision').val()=='0.00' || $(Contenedor + 'txtComision').val()=='')
         {alert ("Ingrese la comision"); return false;}
      
       if (parseFloat($(Contenedor + 'txtComision').val()) > parseFloat($(Contenedor + 'txtTotalFactura').val()))
         {alert ("La comision no debe ser mayor que la venta"); return false;}
...
function F_EliminarRegistro(Fila) {
 try 
    ...
    if ($(lblEstado).text()=="FACTURADO(A)") 
    {alert ("Este documento se encuentra facturado; primero elimine la factura");
    return false;}

    if ($(lblEstado).text()=="CANCELADO") 
    {alert ("Este documento se encuentra cancelado; primero elimine la cobranza y luego elimine la factura");
    return false;}
