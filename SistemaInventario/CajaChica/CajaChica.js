            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        $('#MainContent_lblRedondeo').text(result.split('~')[20]);
                        $('#hfRedondeo').val(result.split('~')[20]);

                        $('#MainContent_ddlMonedaDolares').val('2');
                        $('#MainContent_ddl_responsable').val('23');
                        ...
                        $('#MainContent_txtMontoSolesDepositar').css('background', '#FFFFE0');
                        $('#MainContent_txtMontoDolaresDepositar').css('background', '#FFFFE0');
...
function F_Abrir(Fila) {
    ...
    try {
        ...
        var Permiso = F_PermisoOpcion(CodigoMenu, 777001, '');
...
function F_Cerrar(Fila) {
    ...
    try {
        ...
        var Permiso = F_PermisoOpcion(CodigoMenu, 777002, '');
...
function F_Eliminar(Fila) {
    if (F_PermisoOpcion(CodigoMenu, 777011, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
                ...
                F_AgregarTemporal_NET(arg, function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                     ...
                     if (str_mensaje_operacion == "") {
                        ...
                        $('#MainContent_txtMontoSolesDepositar').val(parseFloat(C));
                        $('#MainContent_txtMontoDolaresDepositar').val(parseFloat(D)); 
...
//                        }
                        ...
                        if (result.split('~')[1]=="LA(S) CAJA(S) YA ESTAN LIQUIDADAS")
                        {
                        F_Buscar();
    ...
    try {
        ...
        var F = '0';
            if ($(Cuerpo + 'txtNroSoles').val() == '')
                ...
                if (Math.abs(parseFloat($(Cuerpo + 'txtMontoSoles').val())-parseFloat($(Cuerpo + 'txtMontoSolesDepositar').val())) > parseFloat($('#hfRedondeo').val()))
                Cadena = Cadena + '<p></p>' + 'El redondeo soles solo puede ser +-'+ $('#hfRedondeo').val();

                if(parseFloat($(Cuerpo + 'txtMontoSoles').val())==0 & parseFloat($(Cuerpo + 'txtMontoSolesDepositar').val())>0 )
                Cadena = Cadena + '<p></p>' + 'No puede colocar monto de deposito soles si no hay monto caja';

                 if (Math.abs(parseFloat($(Cuerpo + 'txtMontoDolares').val())-parseFloat($(Cuerpo + 'txtMontoDolaresDepositar').val())) > parseFloat($('#hfRedondeo').val()))
                Cadena = Cadena + '<p></p>' + 'El redondeo dolares solo puede ser +-'+ $('#hfRedondeo').val();

                if(parseFloat($(Cuerpo + 'txtMontoDolares').val())==0 & parseFloat($(Cuerpo + 'txtMontoDolaresDepositar').val())>0 )
                Cadena = Cadena + '<p></p>' + 'No puede colocar monto de deposito Dolares si no hay monto caja';
...
function F_Grabar() {
    try {
        ...
        $('#MainContent_grvLiquidacion .detallesart').each(function () {
             ...
             if($(lblAplicarSoles_grilla).val()<0 | $(lblAplicarDolares_grilla).val()<0)
             {
             Mensaje='Los montos a liquidar no pueden ser menor a 0'
        ...
        var objParams = {
            ...
            Filtro_MontoSolesDepositar: $('#MainContent_txtMontoSolesDepositar').val(),
            ...
            Filtro_MontoDolaresDepositar: $('#MainContent_txtMontoDolaresDepositar').val(),
...
//                    $('#divLiquidacion').dialog('close');
                    F_Buscar();
                    alertify.log('Las Liquidacion(es) se han grabo con exito');
            ...
            lblAplicarDolares_grilla = chkSi.replace('lblFecha', 'txtTotalDolares');
                    ...
                    Soles=Soles+ $(lblAplicarSoles_grilla).val(),
                    Dolares=Dolares+ $(lblAplicarDolares_grilla).val()
...
function F_ValidarDocumentoFomarles(ControlID) {
    ...
    if ($(chkok_grilla).is(':checked')) {
       $('#MainContent_grvDetalleLiquidacion .detallesart').each(function () {
                    ...
                    lblDocumento = chkSi.replace('lblEmision', 'lblDocumento');

                   // ASIGNO VALOR CERO A LOS DOCUMENTOS NO FORMALES
                    if (($(lblDocumento).text().indexOf('F') == -1 & $(lblDocumento).text().indexOf('B') == -1 & $(lblDocumento).text().indexOf('RH') == -1) | ($(hfcodtipodoc).val() == 2 & parseFloat($(hfSolesAplicado).val())< 0 ) )
                    {
                         $(txtTotalSoles).val('0');
...
//                    }   
            ...
            $('#MainContent_grvDetalleLiquidacion .detallesart').each(function () {
                    ...
                    if ($(lblDocumento).text().indexOf('F') == -1 & $(lblDocumento).text().indexOf('B') == -1)
                       {
...
function F_ValidaMontoDepositoSoles(Fila) {
    try {
        var txtMontoSolesDepositar = '#' + Fila;
        var txtMontoSoles = txtMontoSolesDepositar.replace('txtMontoSolesDepositar', 'txtMontoSoles');
        var hfMontoSoles = txtMontoSolesDepositar.replace('txtMontoSolesDepositar', 'hfMontoSoles');

       
       if(Math.abs(parseFloat($(txtMontoSoles).val())-parseFloat($(txtMontoSolesDepositar).val())) > parseFloat($('#hfRedondeo').val()))
       {
            $(txtMontoSolesDepositar).val(parseFloat($(txtMontoSoles).val()).toFixed(2));
          
            alertify.log("El redondeo soles solo puede ser +-" + $('#hfRedondeo').val()) ;
            return false;
...
function F_ValidaMontoDepositoDolares(Fila) {
    try {
        var txtMontoDolaresDepositar = '#' + Fila;
        var txtMontoDolares = txtMontoDolaresDepositar.replace('txtMontoDolaresDepositar', 'txtMontoDolares');
        var hfMontoDolares = txtMontoDolaresDepositar.replace('txtMontoDolaresDepositar', 'hfMontoDolares');

       
       if(Math.abs(parseFloat($(txtMontoDolares).val())-parseFloat($(txtMontoDolaresDepositar).val())) > parseFloat($('#hfRedondeo').val()))
       {
            $(txtMontoDolaresDepositar).val(parseFloat($(txtMontoDolares).val()).toFixed(2));
          
            alertify.log("El redondeo dolares solo puede ser +-" + $('#hfRedondeo').val()) ;
            return false;
