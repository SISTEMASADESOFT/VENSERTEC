var AppSession = "../Maestros/ProductosWilliam.aspx";
...
$(document).ready(function () {
    ...
    $('#MainContent_btnGrabarConsulta').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        F_Update_Division_HTML('div_FamiliaEdicionConsulta', result.split('~')[8]);
                        ...
                        F_Update_Division_HTML('div_Servicios', result.split('~')[15]);
                        F_Update_Division_HTML('div_ServiciosEdicion', result.split('~')[16]);

                        if (result.split('~')[17] == 1 | result.split('~')[17] == 0 ) {

                            $('#MainContent_ddlservicio').val('0');
                            $('#MainContent_ddlservicioEdicion').val('0');
                            $('#MainContent_ddlservicio').prop('disabled', true);
                            $('#MainContent_ddlservicioEdicion').prop('disabled', true);
                        ...
                        $('#MainContent_ddlFamilia').val('007');
                        ...
                        $('#MainContent_ddlservicio').css('background', '#FFFFE0');
                        $('#MainContent_ddlservicioEdicion').css('background', '#FFFFE0');

                        $('#MainContent_ddlEstado').css('background', '#FFFFE0');
                        $('#MainContent_ddlEstadoEdicion').css('background', '#FFFFE0');
                        $('.ccsestilo').css('background', '#FFFFE0');
                        $('.AutoComplete').css('background', '#FFFFE0');
                        AutocompleteFamilia('#MainContent_grvConsulta_txtFamilia_0');
                        AutocompleteMarca('#MainContent_grvConsulta_txtMarca_0');
...
function F_ValidarGrabarDocumento() {
    ...
    try {
        ...
        if ($(Cuerpo + 'txtFactor').val() == '')
          ...
          if ($(Cuerpo + 'txtStockMinimo').val() == '')
            Cadena = Cadena + '<p></p>' + 'Stock Mínimo';
...
function F_Buscar() {
    ...
    try {
        var objParams = {
            ...
            Filtro_Ubicacion: $('#MainContent_txtUbicacionConsulta').val()
        };
        ...
        F_Buscar_NET(arg, function (result) {
            ...
            if (str_resultado_operacion == "1") {
                ...
                if (str_mensaje_operacion != "")
                    alertify.log(result.split('~')[1]);
                $('#lblGrillaConsulta').text(F_Numerar_Grilla("grvConsulta", 'lblstock'));
                    if ($('#MainContent_ddlFiltroCodEstado').val()==2)
                    {
...
function F_EditarRegistro(Fila) {
    ...
    try {
        ...
        var lblDescripcionIngles = '#' + imgID.replace('imgEditarRegistro', 'lblDescripcionIngles');
         var txtStockMinimoConsulta = '#' + imgID.replace('imgEditarRegistro', 'txtStockMinimoConsulta');

        var Cuerpo = '#MainContent_';

        $(Cuerpo + 'txtCodigoProductoEdicion').val($(lblCodigoProducto_grilla).val());
        $(Cuerpo + 'ddlFamiliaEdicion').val($(hfCodFamilia_grilla).val());
        $(Cuerpo + 'txtDescripcionEdicion').val($(lblProducto_grilla).val());
        ...
        $(Cuerpo + 'txtCostoEdicion').val($(lblCosto_grilla).val());
        ...
        $(Cuerpo + 'txtDescuentoEdicion').val($(lblDescuento).val());
        $(Cuerpo + 'txtMargenEdicion').val($(lblMargen).val());
        ...
        $(Cuerpo + 'txtUbicacionEdicion').val($(lblUbicacion).val());
        ...
        $(Cuerpo + 'txtStockMinimoEdicion').val($(txtStockMinimoConsulta).val());
        $(Cuerpo + 'txtComentarioEdicion').val('');
        var d = new Date();
        var objParams = {
        ...
        F_ConsultaMovimiento_NET(arg, function (result) {
            ...
            if (str_resultado_operacion == "1") {
                ...
                if (F_PermisoOpcion(CodigoMenu, '113002', '') === "1") {
...
function F_EdicionRegistro() {
    ...
    try {
        ...
        var objParams = {
            ...
            Filtro_CodEstadoBusqueda: $('#MainContent_ddlFiltroCodEstado').val(),
            Filtro_StockMinimo: $('#MainContent_txtStockMinimoEdicion').val(),
            Filtro_Comentario: $('#MainContent_txtComentarioEdicion').val()
        };
...
function F_ValidarEdicionDocumento() {
    ...
    try {
        ...
        if ($(Cuerpo + 'txtStockMinimoEdicion').val() == '')
            Cadena = Cadena + '<p></p>' + 'Stock Mínimo';
...
function F_HistorialCostos(Fila) {
    ...
    try {
        ...
        var lblCodigoProducto_grilla = '#' + imgID.replace('imgHistorialCosto', 'txtCodigoProducto');
        var lblProducto_grilla = '#' + imgID.replace('imgHistorialCosto', 'txtProducto');
        var lblMarca_grilla = '#' + imgID.replace('imgHistorialCosto', 'txtMarca');
        ...
        var MarcaProducto = $(lblMarca_grilla).val().trim();
        var TituloProducto = $(lblCodigoProducto_grilla).val() + ' - ' + $(lblProducto_grilla).val();
        ...
        F_ConsultaHistorialCostos_NET(arg, function (result) {
            ...
            if (result.split('~')[1] == "") {
                ...
                $('#lblGrillaConsultaCostos').text(F_Numerar_Grilla("grvHistorialCostos", 'hfFechaRegistro'));
...
function F_DetalleProducto(Fila) {
    ...
    var lblCodigoProducto = '#' + imgID.replace('imgDetalleProducto', 'txtCodigoProducto');
    var lblProducto = '#' + imgID.replace('imgDetalleProducto', 'txtProducto');
    $('#hfCodProducto').val($(lblCodigo).val());
    $(Contenedor + 'txtProductoDetalle').val($(lblProducto).val());
    $(Contenedor + 'txtModeloDetalle').val('');
    $(Contenedor + 'txtAñoDetalle').val('');
    $(Contenedor + 'txtMotorDetalle').val('');
    $(Contenedor + 'txtCajaCambio').val('');
...
function F_ValidarCheck(ControlID) {
    ...
    var txtCodAlterno = chkok_grilla.replace('chkOK', 'txtCodAlterno');
    var txtPais = chkok_grilla.replace('chkOK', 'txtPais');
    var txtCodigoProducto = chkok_grilla.replace('chkOK', 'txtCodigoProducto');
    var txtProducto = chkok_grilla.replace('chkOK', 'txtProducto');
    var txtDscProducto2 = chkok_grilla.replace('chkOK', 'txtDscProducto2');
    var txtMarca = chkok_grilla.replace('chkOK', 'txtMarca');
    var txtModelo = chkok_grilla.replace('chkOK', 'txtModelo');
    var txtMotor = chkok_grilla.replace('chkOK', 'txtMotor');
    ...
    var txtAño = chkok_grilla.replace('chkOK', 'txtMargen');
    var txtDescuento = chkok_grilla.replace('chkOK', 'txtDescuento');
    var txtFamilia = chkok_grilla.replace('chkOK', 'txtFamilia');
    var txtUbicacion = chkok_grilla.replace('chkOK', 'txtUbicacion');
    var txtStockMinimo = chkok_grilla.replace('chkOK', 'txtStockMinimoConsulta');
    ...
    if ($(chkok_grilla).is(':checked')) {
        ...
        $(txtDscProducto2).prop('disabled', false);
        $(txtProducto).prop('disabled', false);
        $(txtCodigoProducto).prop('disabled', false);
        $(txtPais).prop('disabled', false);
        $(txtCodAlterno).prop('disabled', false);
        $(txtFamilia).prop('disabled', false);
        $(txtCodAlterno).select();
        $(txtStockMinimo).prop('disabled', false);
        AutocompleteFamilia(txtFamilia);
        AutocompleteMarca(txtMarca);
 ...
 $(txtUbicacion).prop('disabled', true);
        ...
        $(txtDscProducto2).prop('disabled', true);
        $(txtProducto).prop('disabled', true);
        $(txtCodigoProducto).prop('disabled', true);
        $(txtPais).prop('disabled', true);
        $(txtCodAlterno).prop('disabled', true);
...
function F_ValidarAgregar() {
    try {
        ...
        $('#MainContent_grvConsulta .chkSi :checkbox').each(function () {
            var chkSi = '#' + this.id;
            var txtDscProducto = chkSi.replace('chkOK', 'txtCodigoProducto');
            var txtProducto = chkSi.replace('chkOK', 'txtProducto');
            var txtCosto = chkSi.replace('chkOK', 'txtCosto');
            var txtMargen = chkSi.replace('chkOK', 'txtMargen');
            var txtDescuento = chkSi.replace('chkOK', 'txtDescuento');
            var txtMarca = chkSi.replace('chkOK', 'txtMarca');
            var txtStockMiniomo = chkSi.replace('chkOK', 'txtStockMinimoConsulta');

            if ($(chkSi).is(':checked')) {
                if ($(txtDscProducto).val() == '')
                    cadena = cadena + '<p></p>' + "Descripcion para el codigo " + $(txtDscProducto).val();

                if ($(txtProducto).val() == '')
                    cadena = cadena + '<p></p>' + "Descripcion para el Producto  " + $(txtProducto).val();

                if ($(txtStockMiniomo).val() == '')
                    cadena = cadena + '<p></p>' + "Stock Minimo paa el Producto " + $(txtProducto).val();

//                    if ($(txtMarca).val() == '')
//                        cadena = cadena + '<p></p>' + "Descripcion para el Marca  " + $(txtMarca).val();

//                    if ($(txtDescuento).val() == '')
//                    cadena = cadena + "\n" + "Descripcion para el codigo " + $(txtDescuento).text();

                    if ($(txtCosto).val() == '')
                        cadena = cadena + '<p></p>' + "Falta Costo " + $(txtCosto).val();
                    
                    if ($(txtMargen).val() == '')
                        cadena = cadena + '<p></p>' + "Falta Costo " + $(txtMargen).val();
                    
//                    if ($(txttxtCodAlternoDscProducto).val() == '')
//                    cadena = cadena + "\n" + "Descripcion para el codigo " + $(txtCodAlterno).text();
...
//            txtCodAlterno           = chkSi.replace('chkOK', 'txtCodAlterno');
//            txtPais                 = chkSi.replace('chkOK', 'txtPais');
            txtCodigoProducto       = chkSi.replace('chkOK', 'txtCodigoProducto');
//            txtDescripcionAuxiliar  = chkSi.replace('chkOK', 'txtDescripcionAuxiliar');
            txtProducto             = chkSi.replace('chkOK', 'txtProducto');
//            txtDscProducto2         = chkSi.replace('chkOK', 'txtDscProducto2');
            txtMarca                = chkSi.replace('chkOK', 'txtMarca');
//            txtMotor                = chkSi.replace('chkOK', 'txtMotor');
...
//            txtMEDIDA               = chkSi.replace('chkOK', 'txtMEDIDA');
            ...
            txtUbicacion            = chkSi.replace('chkOK', 'txtUbicacion');
            txtFamilia               = chkSi.replace('chkOK', 'txtFamilia');
            hfCodFamilia             = chkSi.replace('chkOK', 'hfCodFamilia');
            ddlEstado               = chkSi.replace('chkOK', 'ddlEstado');
            txtStockMinimoConsulta = chkSi.replace('chkOK', 'txtStockMinimoConsulta');  

            if ($(chkSi).is(':checked')) {
                var objDetalle = {
                    hfCodProducto         : $(hfCodProducto         ).val(),
//                    txtCodAlterno         : $(txtCodAlterno         ).val(),
//                    txtPais               : $(txtPais               ).val(),
                    txtCodigoProducto     : $(txtCodigoProducto     ).val(),
//                    txtDescripcionAuxiliar: $(txtDescripcionAuxiliar).val(),
                    txtProducto           : $(txtProducto           ).val(),
//                    txtDscProducto2       : $(txtDscProducto2       ).val(),
                    txtMarca              : $(txtMarca              ).val(),
//                    txtMotor              : $(txtMotor              ).val(),
//                    txtPosicion           : $(txtPosicion           ).val(),
//                    txtAño                : $(txtAño                ).val(),
//                    txtMEDIDA             : $(txtMEDIDA             ).val(),
                    txtCosto              : $(txtCosto              ).val(),
                    txtMargen             : $(txtMargen             ).val(),
                    txtDescuento          : $(txtDescuento          ).val(),
                    txtUbicacion          : $(txtUbicacion          ).val(),
                    txtFamilia            : $(txtFamilia             ).val(),
                    hfCodFamilia            : $(hfCodFamilia             ).val(),
                    CodEstado: $(ddlEstado).val(),
                    txtStockMinimo: $(txtStockMinimoConsulta).val()
                };
        ...
        F_AgregarTemporal_NET(arg, function (result) {
            ...
            if (str_mensaje_operacion == "Se Grabo Correctamente")
                F_Update_Division_HTML('grvConsulta', result.split('~')[2]);
...
function AutocompleteFamilia(ControlID) {
 var codfamilia = '';
        var descripcion= '';
$('#MainContent_grvConsulta .AutoComplete').each(function(){
        $('#MainContent_grvConsulta .chkSi :checkbox').each(function () {  
        chkSi = '#' + this.id;
        if ($(chkSi).is(':checked')) {
        familia= chkSi.replace('chkOK', 'txtFamilia');
        hfCodFamilia= chkSi.replace('chkOK', 'hfCodFamilia');
              ...
              $(familia).autocomplete(
            {
                ...
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: '../Servicios/Servicios.asmx/F_Familia_AUTOCOMPLETE',
                data: "{'Descripcion':'" + request.term + "'}",
                dataType: "json",
                async: true,
                success: function (data) {
                ...
                error: function (response) {
                 ...
                 select: function (e, i) {
                     $(familia).val(i.item.label);
                     $(hfCodFamilia).val(i.item.val);
                 },
                 complete: function () {
...
function AutocompleteMarca(ControlID) {
    var lblMarca = '';
    var descripcion = '';
    $('#MainContent_grvConsulta .AutoComplete').each(function () {
        $('#MainContent_grvConsulta .chkSi :checkbox').each(function () {
            chkSi = '#' + this.id;
            if ($(chkSi).is(':checked')) {
                Marca = chkSi.replace('chkOK', 'txtMarca');
                lblMarca = chkSi.replace('chkOK', 'lblMarca');
...
function F_Inicializar_CajaTexto() {
    ...
    $('#MainContent_txtUbicacionConsulta').css('background', '#FFFFE0');
