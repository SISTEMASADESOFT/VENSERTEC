...
var CodigoMenu = 1000; var CodigoInterno = 7;
...
function F_EliminarRegistro(Fila) {
    ...
    try {
        var imgID         = Fila.id;
        var lblIDFamilia  = '#' + imgID.replace('imgAnularDocumento', 'lblIDFamilia');
        var lblDscFamilia = '#' + imgID.replace('imgAnularDocumento', 'lblDscFamilia');

        if (!confirm("ESTA SEGURO DE ELIMINAR LA FAMILIA " + $(lblDscFamilia).text()))
            return false;

        var objParams = {
            Filtro_IDFamilia: $(lblIDFamilia).text(),
            Filtro_DscFamilia: $('#MainContent_txtDescripcionConsulta').val()
        };
...
function F_EditarRegistro(Fila) {
    ...
    try {
        var imgID = Fila.id;
        var lblIDFamilia = '#'  + imgID.replace('imgEditarRegistro', 'lblIDFamilia');
        var lblCodFamilia = '#' + imgID.replace('imgEditarRegistro', 'lblCodFamilia');
        var lblDscFamilia = '#' + imgID.replace('imgEditarRegistro', 'lblDscFamilia');
        var hfCodEstado = '#'   + imgID.replace('imgEditarRegistro', 'hfCodEstado');
        var Cuerpo = '#MainContent_';

        $('#hfIDFamilia').val($(lblIDFamilia).text());
        $(Cuerpo + 'txtCodFamiliaEdicion').val($(lblCodFamilia).text());
        $(Cuerpo + 'ddlEstadoEdicion').val($(hfCodEstado).val());
        $(Cuerpo + 'txtDescripcionEdicion').val($(lblDscFamilia).text());
