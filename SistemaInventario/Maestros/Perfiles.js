...
function F_Ini_OtrosCombos() {
    ...
    F_Conceptos($('#MainContent_ddlEstadoEdicion'), 27);
...
function F_PaginasPermisos_Listar() {
    ...
    var CodUsuario = $('#MainContent_ddlUsuario').val();
    var CodEmpleado = 0;

    if ($('#MainContent_ddlUsuario option:selected').text().indexOf("(SOLO EMPLEADO, NO TIENE  USUARIO)") > 0) {
        CodUsuario = 0;
        CodEmpleado = $('#MainContent_ddlUsuario').val();
    }
    ...
    $.ajax({
        ...
        data: "{'CodUsuario':'" + CodUsuario + "','CodigoMenu':'" + $('#MainContent_ddlMenu').val() + "'}",
        dataType: "json",
        async: true,
        success: function (dbObject) {
            var data = dbObject.d;
...
function F_PaginasPermisos(Lista) {
    try {
        ...
        var html = '<thead><tr> ' +
                            ...
                            '<th style="width:60px">Insertar</th> ' + 
...
var TipoOperacion = '';
var divEdicionRegistro_height = 330;
var divEdicionRegistro_width = 720;
function F_Agregar() {
    if (!F_SesionRedireccionar(AppSession)) return false;
...
function F_Editar() {
    ...
    $('#MainContent_txtNombreUsuario').prop('disabled', true);
    ...
    $('#hfCodUsuario').val(CodUsuario);
    $('#hfCodEmpleado').val(CodEmpleado);
    ...
    $.ajax({
        ...
        data: "{'CodUsuario':'" + CodUsuario + "','CodEmpleado':'" + CodEmpleado + "'}",
        dataType: "json",
        async: false,
        success: function (dbObject) {
            var data = dbObject.d;
            try {
                if (data.MsgError === "") {
                    ...
                    $('#hfCodUsuario').val(data.CodUsuario);
                    $('#hfCodEmpleado').val(data.CodEmpleado);

                    if (data.CodUsuario > 0) {
                        $('#MainContent_chkUsuario').prop("checked", true);
                        $('#MainContent_chkUsuario').prop("disabled", true);
                    } else {
                        $('#MainContent_chkUsuario').prop("checked", false);
                        $('#MainContent_chkUsuario').prop("disabled", false);
                        $('#MainContent_txtNombreUsuario').prop("disabled", false);
                    }

                    if (data.CodEmpleado > 0) {
                        $('#MainContent_chkEmpleado').prop("checked", true);
                        $('#MainContent_chkEmpleado').prop("disabled", true);
                    } else {
                        $('#MainContent_chkEmpleado').prop("checked", false);
                        $('#MainContent_chkEmpleado').prop("disabled", false);                    
                    }
...
function F_Grabar() {
    ...
    var UsuarioCE = {};
    UsuarioCE["CodUsuario"] = Number($('#hfCodUsuario').val());
    UsuarioCE["CodEmpleado"] = Number($('#hfCodEmpleado').val());
    ...
    UsuarioCE["Tipo"] = ($('#MainContent_ddlTipo').val() === null) ? '' : $('#MainContent_ddlTipo').val();
    ...
    UsuarioCE["FlagUsuario"] = ($('#MainContent_chkUsuario').is(':checked')) ? 1 : 0;
    UsuarioCE["FlagEmpleado"] = ($('#MainContent_chkEmpleado').is(':checked')) ? 1 : 0;
    UsuarioCE["FlagEliminarEmpleado"] = ($('#MainContent_chkEliminarEmpleado').is(':checked')) ? 1 : 0;
    UsuarioCE["TipoOperacion"] = TipoOperacion;
...
function F_ValidarGrabar() {
    try {
        ...
        if (!$('#MainContent_chkUsuario').is(':checked') & !$('#MainContent_chkEmpleado').is(':checked')) {
            Cadena = Cadena + '<p></p>' + 'Debe seleccionar usuario y/o empleado';
        }
        ...
        if ($('#MainContent_chkUsuario').is(':checked')) {
            ...
            if ($(Cuerpo + 'ddlTipo').val() == 0 || $(Cuerpo + 'ddlEstadoEdicion').val() == null)
                Cadena = Cadena + '<p></p>' + 'Tipo de Usuario';

            if ($(Cuerpo + 'ddlEstadoEdicion').val() == 0 || $(Cuerpo + 'ddlEstadoEdicion').val() == null)
                Cadena = Cadena + '<p></p>' + 'Estado';
        }

        if ($('#MainContent_chkEmpleado').is(':checked')) {
...
function F_LimpiarCampos() {
    ...
    $('#MainContent_chkUsuario').prop("disabled", false);
    $('#MainContent_chkEmpleado').prop("disabled", false);
    $('#MainContent_chkEliminarEmpleado').prop("checked", false);
...
function F_AbrirDropzone_JS() {
    ...
    var str_id;
    str_id = $('#MainContent_ddlUsuario').val(); if (str_id == '') str_id = '0';
