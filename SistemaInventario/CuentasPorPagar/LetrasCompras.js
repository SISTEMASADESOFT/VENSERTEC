var AppSession = "../CuentasPorPagar/LetrasCompras.aspx";
        ...
        var arrDetalle = new Array();
                ...
                F_AgregarTemporal_NET(arg, function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        $('#lblCantidadRegistroF').text(F_Numerar_Grilla("grvFactura", "lblID"));
       ...
       try 
        {
              ...
              var objParams = {
                ...
                if (str_resultado_operacion == "1") 
                {
                    ...
                    $('#lblGrillaConsulta').text(F_Numerar_Grilla("grvDetalleArticulo", "lblCodigo"));                         
                }
                else 
                {
                        toastr.warning(result.split('~')[1]);
                    ...
                    if (str_mensaje_operacion == 'LAS LETRAS SE HAN AGREGADO CON EXITO')
                       {
                           ...
                           $('#lblCantidadRegistroL').text(F_Numerar_Grilla("grvLetra", "lblCodigo"));
...
function F_EliminarRegistro(Fila) {
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Eliminar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
        if ($('#hfCodUsuarioAuxiliarAnulacion').val() == '0') {
            ...
            $('#div_UsuarioEliminacion').dialog('open');
        ...
        try {
            ...
            if ($(lblEstado).text() == "CANCELADO PARCIAL") {
                toastr.warning("ESTA LETRA SE ENCUENTRA CANCELADA PARCIAL; PRIMERO ELIMINE LA COBRANZA Y LUEGO ELIMINE LA LETRA");
                return false;
            }

            if ($(lblEstado).text() == "CANCELADO TOTAL") {
                toastr.warning("ESTA LETRA SE ENCUENTRA CANCELADA TOTAL; PRIMERO ELIMINE LA COBRANZA Y LUEGO ELIMINE LA LETRA");
                return false;
            }
            ...
            var objParams = {
                            ...
                            Filtro_ChkCliente: chkCliente,
                            Filtro_CodSede: $('#hfCodSede').val(),
                            Filtro_Observacion: 'ELIMINADO DESDE EL FORMULARIO LETRAS DE COMPRAS'
            };
...
function F_GrabarCodigo() {
    ...
    function F_Proforma(Fila) {
        if (F_PermisoOpcion(CodigoMenu, 6000303, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
        try {
            var imgID = Fila.id;
