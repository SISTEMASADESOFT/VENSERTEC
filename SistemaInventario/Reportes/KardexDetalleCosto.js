...
function F_Reporte(HlkControlID) {
    var Contenedor = '#cphCuerpo_';
  
    CodProducto = $('#' + HlkControlID.replace('hlkCodNeumaticoGv', 'hfCodProducto')).val();

    var rptURL = '';
    var Params = 'width=' + (screen.width) + ', height=' + (screen.height) + ', top=0, left=0, directories=no, menubar=no, toolbar=no, location=no, resizable=yes, scrollbars=yes, titlebar=yes';
    var CodMenu = '3';
    var Titulo = 'Kardex Detalle Costo ' + $.now();
    var TipoArchivo = 'application/pdf';
    var CodTipoArchivo = '5';

    rptURL = '../Reportes/Crystal.aspx';
    rptURL = rptURL + '?';
    rptURL = rptURL + 'NroMesAnio=' + $('#MainContent_txtPeriodo').val() + '&';
    rptURL = rptURL + 'CodProducto=' + CodProducto + '&';
    rptURL = rptURL + 'CodMenu=' + CodMenu + '&';
