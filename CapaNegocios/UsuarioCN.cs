 ...
 public   class UsuarioCN
    {
        ...
        public List<UsuarioCE> F_Usuario_Listar(int CodAlmacen, int CodEstado)
        {
            try
            {
        ...
        public UsuarioCE F_UsuarioEmpleado_Obtener(int CodUsuario, int CodEmpleado)
        {
            UsuarioCE cDatos = new UsuarioCE();
            try
            {
                DataTable dtDatos = obj.F_UsuarioEmpleado_Obtener(CodUsuario, CodEmpleado);
                foreach (DataRow r in dtDatos.Rows)
                {
                    cDatos = new UsuarioCE()
                    {
                        CodUsuario = Convert.ToInt32(r["CodUsuario"].ToString()),
                        CodEmpleado = Convert.ToInt32(r["CodEmpleado"].ToString()),
                        NombreUsuario = r["NombreUsuario"].ToString(),
                        Clave = r["Clave"].ToString(),
                        Apellidos = r["Apellidos"].ToString(),
                        Nombre = r["Nombre"].ToString(),
        ...
        public DataTable F_UsuariosPermisos_Verificar(int CodUsuario, int CodigoMenu, int CodigoInterno)
        {
            try
            {
                return obj.F_UsuariosPermisos_Verificar(CodUsuario, CodigoMenu, CodigoInterno);
            }
            catch (Exception ex)
            {
        ...
        {
            if (objEntidad.TipoOperacion == "Insertar")
