  ...
  public  class TCAlmacenCD
    {
      ...
      public DataTable F_TCALMACEN_LISTAR_TODOS_william(TCAlmacenCE objEntidad)
      {
          #region VARIABLES
          DataTable dta_consulta = null;
          ...
          try
          {
              using (SqlConnection sql_conexion = new SqlConnection())
              {
                  ...
                  using (SqlCommand sql_comando = new SqlCommand())
                  {
                      sql_comando.Connection = sql_conexion;
                      sql_comando.CommandType = CommandType.StoredProcedure;
                      sql_comando.CommandText = "PA_TCALMACEN_LISTAR_TODOS_william";

                      //if (objEntidad.CodEstado > 0)
                          sql_comando.Parameters.Add("@CodEstado", SqlDbType.VarChar,1).Value = objEntidad.Estado;
